...
| `home/private_dot_config/cspell/` | one symlink: `dict` → `~/Sync/AppSettings/cSpell/` (the four cspell word lists — personal, tech, Swedish, customer names; private and synced, not in this repo). VS Code's `cSpell.customDictionaries` points at `~/.config/cspell/dict/…` on every machine (Settings Sync carries the setting; the Linux workstation's own chezmoi repo puts real files at the same path) |
